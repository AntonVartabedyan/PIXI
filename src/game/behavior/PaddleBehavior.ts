...
export class PaddleBehavior extends GameObjectBehavior {
    ...
    private onBrickHit(e: any) {
        ...
        if(this.ball.justHit) {
            return;
        }

        this.ball.justHit = true;
        ...
        if(e.brickType === BrickType.TYPE_3) {
            ...
            // this.paddle.VELOCITY = 30;
            // this.paddleImg.tint = 0xff0000;
            // this.paddleRef.width += 10;
            if(this.ball.timeoutId){
                clearTimeout(this.ball.timeoutId);
            }
            this.ball.timeoutId = setTimeout(() => {
                this.VELOCITY = 15;
