...
export class GameApplication extends PIXI.Application {
    ...
    private ang: number = Math.ceil(Math.random() * 360);
    ...
    private createBall() {
        ...
        this.ball.x = 500;
        this.ball.y = 300;
        // this.ball.rotation = Math.ceil(Math.random() * 360)
    ...
    private onTick(delta: number) {
        ...
        if (this.btn2Down) {
            ...
            if ((scaleX <= (GameApplication.STAGE_WIDTH - (this.ball.width + this.ball.x))) && (scaleY <= (GameApplication.STAGE_HEIGHT - (this.ball.height + this.ball.y)))) {
